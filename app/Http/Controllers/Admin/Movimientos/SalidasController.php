...
class SalidasController extends Controller
{
        ...
        if (empty($request->input('search'))) {
            ...
            if ($request->input('tipo') == 'TODOS') {
                ...
                $movimientos = Movement::join('stores', 'movements.to', '=', 'stores.id')
                    ->join('customers', 'movements.to', '=', 'customers.id')
            ...
            } elseif ($request->input('tipo') == 'CLIENTES') {

                $arrTypes        = ['VENTACLIENTE'];
                    ...
                    if (isset($movimiento->invoice) && count($movimiento->invoice)) {
                        ...
                        foreach ($movimiento->invoice as $invoice) {
                            if ((!is_null($invoice->cae) && !is_null($invoice->url) && $movimiento->status == 'FINISHED') ||
                                ...
                                $number = ($invoice->cyo) ? 'CyO - ' . $invoice->voucher_number : $invoice->voucher_number;
                                $urls .= '<a class="text-primary" title="Generar Comprobantes" target="_blank" href="' . route('ver.fe', ['movment_id' => $movimiento->id,'pto_vta' => false, 'cyo' => false,'invoice_id' => $invoice->id]) . '">' . $number . ' </a><br>';
                            }
                        }
                        $factura = $urls;
    ...
    public function pendientePrint(Request $request)
    {
        ...
        $session_products = DB::table('session_products as t1')
            ...
            ->select(
                ...
                't2.unit_weight',
                't2.unit_type',
                't2.unit_package as presentacion'
            )
    ...
    public function getFleteSessionProducts(Request $request)
    {
        try {
            ...
            // Categoria = 1 Indica que es PRODUCTO CONGELADO
    ...
    public function storeSessionProduct(Request $request)
    {
        try {
            ...
            if (\Auth::user()->rol() == 'contable' && ($a_deposito == $desde_deposito) && isset($a_deposito, $desde_deposito)) {
    ...
    private function checkIfNoCongelado($products)
    {
        foreach ($products as $product) {
            $categoria = Product::find($product->product_id)->categorie_id;
            if ($categoria > 1) {
                return 2;
            }
        }
            ...
            foreach ($session_products as $product) {
                ...
                if ($insert_data['type'] != 'VENTACLIENTE') {
                    ...
                    if ($a_deposito) {
                        ...
                        if ($prod_a_depsito) {
                            if (isset($quantities[0])) {
                                $prod_a_depsito->stock_f += $cant_total_f;
                            }
                            if (isset($quantities[1])) {
                                $prod_a_depsito->stock_r += $cant_total_r;
                            }
                            if (isset($quantities[2])) {
                                $prod_a_depsito->stock_cyo += $cant_total_cyo;
                            }
                            $prod_a_depsito->save();
                        } else {
                            ...
                            if (isset($quantities[0])) {
                                $data_prod_a_deposito['stock_f'] = $cant_total_f;
                            }
                            if (isset($quantities[1])) {
                                $data_prod_a_deposito['stock_r'] = $cant_total_r;
                            }
                            if (isset($quantities[2])) {
                                $data_prod_a_deposito['stock_cyo'] = $cant_total_cyo;
                            }
    ...
    public function updateStockStores($store_id = false, $cod_fenovo = false)
    {
        ...
        if ($store_id) {
            $stores = Store::whereId($store_id)->select('id')->get();
        } else {
        ...
        foreach ($stores as $store) {
            ...
            foreach ($products as $p) {
                ...
                for ($i = 0; $i < count($movements_products); $i++) {
                    ...
                    if (in_array($tienda_id, [11, 59, 60]) && $m->type == 'AJUSTE' && $m->to == $tienda_id && $m->from == $tienda_id && $m->voucher_number == '00001') {
        ...
        return "Tiempo ejecucion {$hours} : {$minutes} : {$seconds}";
    }
